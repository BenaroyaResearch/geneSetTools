#' Determine median percentile ranks of a set of genes in a counts object
#' 
...
#' @export
#' @usage \code{gene_set_median_percrank(gene_set, counts,
...
gene_set_median_percrank <-
  function(gene_set, counts,
    ...
    percranks <-
      ...
      t() %>%
      apply(MARGIN=2, function(x) {rank(x)/length(x)})
    ...
    if (length(gene_set) == 1) {
      ...
      median_percranks <-
        apply(percranks, MARGIN=1, median)
    }
